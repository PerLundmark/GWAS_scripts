...
scripts = script_dir #where to pick up other R scripts

# Settings previously from home dir script settings file (review_settings.R)
# TODO: Move to script parameters to set from NF script
...
cat(paste("  Considering chromosomes", paste(chromosomes,collapse = ' '), "\n"))
...
#TODO: Add step to filter on parameters with plink to generate output for script, or rewrite completely?
...
CMplot(manhat, plot.type = "d", bin.size = 1e6, chr.den.col = c("darkgreen", "yellow", "red"), memo="", dpi = 300,
       file.output = FALSE, verbose = FALSE, width = 12, height = 8)
