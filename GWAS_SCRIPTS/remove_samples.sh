...
## +++ Call:  
#
# remove_samples --genoid MF2 --new_genoid MF3 --sfile samples_to_remove.txt  # /proj/sens2019016/GWAS_TEST 
...
if [ "$#" -lt 6 ]; then
  ...
  echo "         -s|--sfile <file>              no default"    # samples to remove from genotype dataset 
...
log="${genoid}_${new_genoid}_remove.log"   # master logfile 
echo ""  > ${log}
echo ""   | tee -a ${log}
START=$(date +%s)      
echo -n "  "  | tee -a ${log}
