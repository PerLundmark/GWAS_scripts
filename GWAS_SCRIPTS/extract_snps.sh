...
# HERE, we select for variants but keep the complete set of samples 
# 
#  https://www.cog-genomics.org/plink/2.0/filter      "Input filtering"
#     keep/remove refers to samples    ( --keep option ) 
#     extract/exclude refers to variants(markers)  ( --extract option )  
...
# 
# extract_snps --genoid FTD_rand  --new_genoid FTD30  --sfile snps12345.txt



## +++ Hardcoded settings & and defaults 

setfile=~/extract_settings.sh     # OBS!: the same as for extract_samples.sh
if [ -s "${setfile}" ];then
...
log="${genofolder}/${genoid}_${new_genoid}_snp_extract.log"   # master logfile 
echo ""  > ${log}
echo ""   | tee -a ${log}
START=$(date +%s)      
echo -n "  "  | tee -a ${log}
...
# required format of input file (variants):
# [umenzel@sens2019016-bianca UKB_PGEN]$ head -5 chr12_markers_failed.txt
# rs555632218
# rs371304015
# rs535094775
# rs60886875
