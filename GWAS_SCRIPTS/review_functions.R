...
link_nearest_gene <- function(row)  {
  ...
  res = list()
  gene_name = gene_row[1,2]    				# LOC101927499  ... but this might be empty! 
  if(gene_name == "") gene_name = gene_row[1,1]
  res["name"] = gene_name  				# res[["name"]] --> "MLOC101927499" 
  res["chrom"] = marker_chrom  				# res[["chrom"]] --> 22  integer
  res["start"] = gene_row[1,4]          		# start position of the nearest gene
  res["end"] = gene_row[1,6]            		# end position of the nearest gene
  return(res)
  # res contains name, chromosome, start and end position of the nearest gene for the marker contained in input frame "row"
}
